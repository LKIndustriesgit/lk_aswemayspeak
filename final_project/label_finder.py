...
import random
from spacy.lang.fo.tokenizer_exceptions import capitalized

nlp = spacy.load('en_core_web_md')

text = open("../week5/books/The Jungle Book").read()
full_doc = nlp(text)
...
capitalized_nouns = [
    token.lemma_.lower()
    for token in anonymized_doc
    if (token.pos_ == "NOUN"
        and token.text[0].isupper()
        and token.lemma_.lower() not in STOP_WORDS
        and token.lemma_.lower() not in generic_replacements["PERSON"])
]
cap_counter = Counter(capitalized_nouns)
print("\n most common caps: \n")
print(cap_counter.most_common(20))
